...
module.exports = {
  ...
  fetchBycategory: (req, callback) => {
    sql = mysql.format(
      "SELECT conference.* FROM conference,category WHERE category.ID = ? AND conference.category = category.ID",
      [req.params.id]
    );
    return conn.query(sql, callback);
  },
  ...
  upload: (req, callback) => {
    ...
    sql = mysql.format(
      "INSERT INTO conference(category,name,date,attached_file,content,host,status) VALUES(?,?,?,?,?,?,?)",
      [category, name, date, attached_file, content, host, status]
    );
    return conn.query(sql, callback);
  },
  addAbsentees: (req, callback) => {
    sql = mysql.format(
      "INSERT INTO absentees SET ? ",
      [req.body]
    );
    return conn.query(sql, callback);
  },
