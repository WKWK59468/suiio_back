...
const getAuthNum = (auth) => {
  const syntax =
    "SELECT COUNT(`permission`) AS 'num' FROM `officer` WHERE `permission` = ?;";
  const params = [auth];
  const field = "num";
...
module.exports = {
  ...
  check: (sID, callback) => {
    const sql = mysql.format(
      "SELECT COUNT(*) AS total FROM member WHERE sID = ?",
      sID
    );
  ...
  add: (req, callback) => {
    ...
    const sql = mysql.format(
      "INSERT INTO `officer`(`sID`, `position`) VALUES(?, ?);",
      values
    );
    return conn.query(sql, callback);
  },
  ...
  delete: async (req, callback) => {
    ...
    if (bool) throw new Error("Can't delete. Permission problem.");
    const values = [data.sID, data.position];
    const sql = mysql.format(
      "DELETE FROM `officer` WHERE `sID` = ? AND `position` = ?;",
      values
    );
    return conn.query(sql, callback);
  },
  ...
  fetchAll: (req, callback) => {
    const sql = mysql.format(
      "SELECT * FROM `officer` ORDER BY `permission` DESC;"
    );
    return conn.query(sql, callback);
  },
  // fetch by position
  fetchByPosition: (req, callback) => {
    ...
    const sql = mysql.format(
      "SELECT * FROM `officer` WHERE `position` = ?;",
      values
    );
    return conn.query(sql, callback);
  },
  ...
  fetchByPermission: (req, callback) => {
    ...
    const sql = mysql.format(
      "SELECT * FROM `officer` WHERE `permission` = ?;",
      values
    );
    return conn.query(sql, callback);
  },
  ...
  updateOfficer: (req, callback) => {
    ...
    const values = [data.sID, data.position];
    const sql = mysql.format(
      "UPDATE `officer` SET `sID` = ? WHERE `position` = ?;",
      values
    );
    return conn.query(sql, callback);
  },
  ...
  updatePermission: (req, callback) => {
    ...
    if (organize.length == 0 || finance.length == 0 || conference.length == 0)
      throw new Error(
        "Can't update. Each permission must be no less than 1 person."
      );
    if (organize.length > 2 || finance.length > 2 || conference.length > 2)
      throw new Error(
        "Can't update. Each permission must be no more than 2 people."
      );
    const auth = {
      organize: "組織負責人",
      finance: "財務負責人",
    ...
    const sql =
      "UPDATE officer SET permission = CASE " +
      Object.keys(auth).reduce((str, key) => {
        return (
          str +
          data[key].reduce((x, pos) => {
