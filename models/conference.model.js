...
module.exports = {
    ...
    fetchOne: (req, callback) => {
        sql = mysql.format('SELECT conference.name,category.name AS category,conference.date,conference.host,conference.content FROM conference,category WHERE conference.ID = ? AND conference.category = category.ID', [req.params.id]);
        return conn.query(sql, callback);
    },
