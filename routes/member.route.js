...
router.route("/login").post((req, res, next) => {
  ...
  jwt.verify(token, SECRET, (jwterr, decoded) => {
    jwterr ? next() : res.status(200).json({ result: "islogin" });
  });
...
router
  .route("/update/member")
  .post(UserController.check, UserController.patchUser);
router
  .route("/update/password")
  .post(UserController.check, UserController.patchPassword);
